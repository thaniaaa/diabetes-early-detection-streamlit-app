...
def user_input(X):
    ...
    with col1:
        ...
        polyuria = st.selectbox("Polyuria", choice)
        st.divider()
        
        st.write(" #### Polydipsia")
        st.write("Sering merasa haus")
        polydipsia = st.selectbox("Polydipsia", choice)
        st.divider()

        st.write(" #### Sudden weight loss")
        st.write("Penurunan berat badan secara tiba-tiba")
        sudden_weight_loss = st.selectbox("Sudden weight loss", choice)
        st.divider()

        st.write(" #### Weakness")
        st.write("Merasa lemah, letih dan lelah")
        weakness = st.selectbox("Weakness", choice)
        st.divider()

        st.write(" #### Polyphagia")
        st.write("Sering merasa lapar")
        polyphagia = st.selectbox("Polyphagia", choice)
        st.divider()

        st.write(" #### Visual blurring")
        st.write("Penglihatan kabur")
        visual_blurring = st.selectbox("Visual blurring?", choice)
        st.divider()

    with col2:
        st.write(" #### Genital Thrush")
        st.write("Infeksi jamur Candida pada area genital. ")
        genital_thrush = st.selectbox("Genital thrush?", choice)
        st.divider()

        st.write(" #### Itching")
        st.write("Gatal")
        itching = st.selectbox("Itching?", choice)
        st.divider()

        st.write(" #### Irritability")
        st.write("Mudah marah pada hal kecil")
        irritability = st.selectbox("Irritability?", choice)
        st.divider()

        st.write(" #### Delayed Healing")
        st.write("Waktu penyembuhan luka lama")
        delayed_healing = st.selectbox("Delayed healing?", choice)
        st.divider()

        st.write(" #### Partial paresis")
        st.write("Kelumpuhan sebagian")
        partial_paresis = st.selectbox("Partial paresis?", choice)
        st.divider()

        st.write(" #### Muscle stiffness")
        st.write("Kekakukan otot")
        muscle_stiffness = st.selectbox("Muscle stiffness?", choice)
        st.divider()

        st.write(" #### Alopecia")
        st.write("Rambut rontok")
        alopecia = st.selectbox("Alopecia?", choice)
        st.divider()

        st.write(" #### Obesity")
        st.write("Berat badan berlebih")
        obesity = st.selectbox("Obesity?", choice)
        st.divider()
